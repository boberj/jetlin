...
**Status:** phases 1–6 implemented, and §11 examined and scoped down rather than built — see its status
line and the last rows of §13. `:samples:demo` was ported and then deliberately put back on its own
in-memory store, which those rows also cost out, because that is the part worth keeping. `./gradlew build`
is green — 359 tests in the main build and 10 in the migration tooling's included build — and the browser
suite is 35/36: the same 35/36 it was before any of this work, the one failure being pre-existing and
unrelated. Nothing from phases 1–6 is outstanding.
...
**Status:** examined, costed, and deliberately not built. The essential part — a value that arrives late,
held as snapshot state — shipped as `jetlin.runtime.Fetch` and `jetlin.runtime.rememberAction`, with a
worked adapter in `:samples:teams`; `docs/architecture.md` §8 documents it. The two modules this section
designs (`:jetlin-data`, `:jetlin-remote`), the shared gate and the authority dimension are **not** built,
and the last rows of §13 say why and name the three things that would change that. Read this section as a
design that was priced rather than one that is pending.
...
| 2026-09-13 | **§11 is examined, costed and not built.** The `:jetlin-data` extraction, `:jetlin-remote`, the shared gate and `CellKey`'s authority dimension are all deliberately absent. | Most of what the two modules would share is shared by choice, not necessity, and every justification for the *identity map* fails for external data: the process does not own it, it does not fit in memory, traversal is a network call regardless, and it goes stale (§2.2's four premises, all false here). Two findings settle it. **The §11.5 hazard is created by the shared map** — Bob reads a resource fetched with Alice's token only because we chose to cache external data in a process-wide map; hold one holder per principal and there is nothing to retrofit. And **"one graph" does not survive the schema**: `reference()` is persisted as a foreign key, so a stored reference cannot point outside the database. You store the external key and look it up, which needs a lookup function on the adapter rather than a shared identity map. What is left of the shared part is `Policy` implemented twice against one interface — real, but not worth a module split, a package rename, a dozen broken `internal`s and the fifteen corrections the design needed. |
| 2026-09-13 | **What was built instead: `Fetch`/`Fetched` and `rememberAction`/`Run`, in `:jetlin-runtime`.** No new module. | The essential shared thing is a value that arrives late held as snapshot state, so that every session which read it recomposes — which is the cell plus `GlobalSnapshotManager`, both of which already existed. Neither type names a database, an HTTP client or an element, and `:jetlin-runtime` already owns the composable helpers, so a new module would have bought nothing. The content is the three ways to get this wrong, each now a comment at the line that prevents it: **the read writes no snapshot state** (an in-flight marker in a cell invalidates its own reader every pass — a session that never settles, which is why two of the tests would hang rather than fail if it regressed); **the fetch runs off the session's confined thread**; and **the arrival is one write**, so one response is one recomposition and one patch however many fields it filled. |
| 2026-09-13 | **An `Action` takes no argument**, and a failed one is `Run.Failed` rather than an `error` field beside a `running` flag. | The plan had `Action<A, R>` with `rememberAction` overloaded on lambda arity; the overloads are ambiguous, because a lambda with no declared parameters matches both. Removing the argument turned out to be the better API anyway: the block is re-read on every composition, so whatever the work needs it reads from state when it runs — and a list wants one action per row (remembered inside the row's `key`), because one shared action would disable every button when any one of them was working. The sealed `Run` replaces `running`/`error`/`result` for the reason `Fetched` is sealed: three fields can describe states that cannot happen, and one field makes a failure one write, so the button coming back and the message appearing are one patch. |
| 2026-09-13 | **Staleness, decided**: a stale value is served while it revalidates, there is no `Stale` state, and nothing retries on its own. | §11.6 left the question open. Flipping a `Ready` cell back to `Loading` on expiry reintroduces exactly the placeholder flicker §2.2 and §2.3 exist to avoid, and throws away something true for nothing; a revalidation that fails keeps the value it had, for the same reason. The TTL is a bound on what a *read* will accept, not a refresh interval: **nothing wakes up when a value expires**, because a read happens only when the composable that reads it recomposes. A page sitting still keeps showing an expired value until something brings it round — another cell, a click, a navigation, a hibernated session waking. `refresh()` is there for a caller who knows somebody is looking (a command that just changed the thing, a retry button, a poller in a `LaunchedEffect` that lives exactly as long as the page); `invalidate()` marks it stale and lets the next reader pay, and the two exist separately because polling for a value nobody is looking at is the cost this design is avoiding. Both a success and a failure record when they happened, so a broken endpoint cannot turn a rendering page into a request per recomposition — which means a failed fetch under the default infinite TTL stays failed until `invalidate()`, and a page that shows "unavailable" should offer a retry that calls it. A revalidation that finds an equal value recomposes nobody, which `LastWriteWins.equivalent` gives for free and a test pins. |
| 2026-09-13 | **`Record.stored` is gone; `Record.database != null` is the same fact.** | The two were set and cleared in exactly the same three places — `commit`'s insert and delete loops and the loader — and `Db.kt`'s own comment called them "both flags". Two fields encoding one fact, and the collapse changed no test, which is the evidence it was a simplification rather than a change. Worth doing independently of anything in §11. |
| 2026-09-13 | **The rule held in place of the extraction: there is never a second authorization model.** The trigger to revisit §11 is one of three things, and nothing else. | The outcome worse than either answer is `:jetlin-remote` growing a parallel `Policy` that drifts from this one. So: (1) something outside `:jetlin-db` needs framework-enforced authorization — a rule an application writes, rather than a cache key that answers the question structurally; (2) an application wants to cache external data across principals, which is the moment `Authority` stops being speculative; (3) a page genuinely needs to treat records and external resources through one type, not merely to render both. Any of those is the second implementation §11.7 says the extraction needs. The `:samples:teams` adapter is the evidence that none of them is in play yet: its per-principal cache is keyed by the principal, so there is no object for the wrong principal to reach, and its shared value needs no rule because it is the same for everyone. |
| 2026-09-13 | **§11.8's acceptance criteria, re-mapped to what exists.** | 1 (two principals, separate data) — proven in `HubTest` over real HTTP, with the stub counting what it was asked, because the claim is about a request that did not happen. 2 (a placeholder without blocking; the arrival is one op) — proven in `FetchTest` by `changeCount`, plus the same shape rendered in the sample. 3 (a failed command changes nothing and does not kill the session) — `ActionTest` and `HubTest`; the `try` is inside the launched coroutine, because an exception escaping there cancels the composition's scope and takes the session with it. 4 (an external call inside `transact` does not compile) — a type-level fact, `transact` taking `() -> T`, documented rather than asserted, following the §4.8-item-3 precedent that this module cannot assert things about application code. 5 (`:jetlin-data` does not depend on `:jetlin-db`) — moot while there is one module. Also worth saying plainly, in the sample and here: its "external system" is a stub in the same process. A fixture with a real HTTP boundary, not a third-party API. |
| 2026-09-13 | **`Fetch.fresh(every)` is the polling story: watching is counted on the value, not on the session.** | The question a page actually has is "keep this current while I am showing it", which `collectAsStateWithLifecycle` answers on Android by tying a collection to a lifecycle. Here the composition *is* the lifecycle, so a `DisposableEffect` is the whole of it — and because the value is shared between sessions, the loop belongs to the value: `watch` reference-counts, one loop serves every watcher, the shortest interval wins, and the loop stops when the last page showing it goes away (including a session that hibernates, whose composition is torn down). Ten people watching a dashboard cost what one costs. The alternative — a loop per session — would multiply requests by the number of people looking, which is the one thing a shared cache exists to avoid. |
