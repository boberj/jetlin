...
**Status:** phases 1–6 implemented, except the `:samples:demo` port — see the last rows of §13.
`./gradlew build` is green: 349 tests in the main build plus 10 in the migration tooling's included build.
...
1. **Ease of use.** One mental model, no cache to invalidate, no query language to learn for the
   common case.
2. **Row-level access control** covering three shapes: owner-only, shared by some property such as a
...
that iterated a team-filtered collection, and the shared rows disappear from that user's open page
...
enforces it. Where "write the row and call the API" must be atomic, use an outbox — record the intent
